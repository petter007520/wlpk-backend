...
//包邮地区地址
var shipping_temp_area = {};
var shipping_submit_data = {};
var shipping_selected_area = {};
var shipping_surplus_area_ids = $("#shipping_surplus_area_ids").val();
var shipping_surplus_area = {};

//地区位置标识
var shipping_area_target = [0];
//操作类型
var shipping_opt_type = 1;
//当前操作序号
var shipping_opt_num = 0;
//总操作序号
var shipping_opt_total = 0;
...
layui.use(['form', 'layer'], function () {
	...
	(function () {
		if (template_id > 0) {
			...
			shipping_opt_total = Number($("#shipping_opt_total").val());
		}
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (template_id == 0) {
					shipping_surplus_area = copy_obj(data);
				} else {
					shipping_surplus_area = get_data_by_ids(data, shipping_surplus_area_ids);
					shipping_submit_data = initshippingSubmitData();
					for (var num in shipping_submit_data) {
						shipping_selected_area[num] = get_data_by_ids(data, shipping_submit_data[num]['area_ids']);
					}
				}
				//改变地区加载标记 关闭弹框 重新渲染数据
				area_data_flag = true;
				if (layer_load) {
					layer.close(layer_load);
					if (shipping_opt_type == 2) {
						// 将临时数据对象清零并重新赋值
						shipping_temp_area = new Object();
						shipping_temp_area = copy_obj(shipping_selected_area[opt_num]);
					}
	...
	function initSurplus(data) {
		...
		$('#distributionArea .area_ids').each(function (i, e) {
	...
	function compile_new_data() {
		...
		$(".express-area-modal .all-area .box").html(surplus_html);
		var temp_html = compile_list(temp_area, 1, 0, '');
		$(".express-area-modal .selected-area .box").html(temp_html);
	}
	...
	function copy_obj(obj) {
		...
		for (var key in obj) {
			var val = obj[key];
			newObj[key] = typeof val === 'object' ? copy_obj(val) : val;
		}
		return newObj;
	}
	...
	function deal_data(area_list, key) {
		let temp = {};
		for (let level in area_list) {
			let level_temp = {};
			for (let pid in area_list[level]) {
				let pid_temp = {child_list: {}};
				let total_num = 0;
				let del_num = 0;
				for (let id in area_list[level][pid]['child_list']) {
	...
	function compile_table() {
		...
		for (var index in submit_data) {
			if (count_obj(submit_data[index]) > 0 && submit_data[index]['area_names']) {
				...
				html += '<span class="right-opt"><span class="opt-update express-opt-update text-color" data-selected="' + index + '">修改</span>&nbsp;<span class="opt-delete text-color" data-selected="' + index + '">删除</span></span>';
				html += '</p>';
				html += '</td>';
		...
		html += '<tr>';
		html += '<td colspan="5" class="text-color add-distribution js-add-record">指定可配送区域和运费</td>';
		html += '</tr>';
	 ...
	 */
	$("body").on('click', '.express-area-list .area-btn', function () {
	 ...
	 */
	$("body").on('click', '.express-area-list.all-area ul li .title-div .name-div', function () {
		var li = $(this).parent().parent();
	 ...
	 */
	$("body").on("click", ".express-area-modal .add", function () {
		// 找到新选中部分
	 ...
	 */
	$("body").on("click", ".express-area-modal .express-area-list.selected-area .area-delete", function () {
	...
	function get_modal_html() {
		var modal_html = '';
		modal_html += '<div class="area-modal express-area-modal">' +
			'<div class="area-list express-area-list all-area">' +
			'<div class="title ">可选省、市、区</div>' +
			'<div class="box"></div>' +
			'</div>' +
			'<button class="add">添加</button>' +
			'<div class="area-list express-area-list selected-area">' +
			'<div class="title ">已选省、市、区</div>' +
			'<div class="box"></div>' +
			'</div>' +
			'</div>' +
			'<div class="modal-operation">' +
			'<button class="layui-btn save-btn express-save-btn">确定</button>' +
	 ...
	 * 开始修改操作
	 */
	$("#distributionArea").on("click", ".express-opt-update", function () {
		// 操作类型为添加
		opt_type = 2;
		// 确定正在操作的数据序列
	 ...
	 * 保存操作
	 */
	$("body").on("click", ".express-save-btn", function () {
	 ...
	 * 取消操作
	 */
	$("body").on("click", ".express-area-modal .cancel-btn", function () {
		layer.close(layer_index);
	});
	...
	$("#distributionArea").on("click", ".opt-delete", function () {
		...
		layer_index = layer.confirm('确定要删除该记录吗', {
			...
			alter_data_attr(selected_area[opt_num], 'choosed', 0);
			// 归还数据到总数据
			...
			// submit_data[opt_num] = new Object();
			// 删除当前行
			$("#distributionArea tr[data-selected='" + opt_num + "']").remove();
			layer.close(layer_index);
		}, function () {
	...
	// 是否默认
	form.on('switch(appoint_free_shipping)', function (e) {

		if (e.elem.checked) {
			$('.free-shipping').removeClass('layui-hide');
		} else {
			$('.free-shipping').addClass('layui-hide');
		}
	});
	...
	form.on('submit(save)', function (form_data) {
		...
		//过滤数据
		var shipping_real_data = new Object();
		for (var per in shipping_submit_data) {
			if (count_obj(shipping_submit_data[per]) > 0) {
				shipping_real_data[per] = shipping_submit_data[per];
			}
		}
		//拼接数据
		for (var i in shipping_real_data) {
			shipping_real_data[i]['snum'] = shipping_real_data[i]['other']['snum'];
			shipping_real_data[i]['sprice'] = shipping_real_data[i]['other']['sprice'];
			// delete shipping_real_data[i]['other'];
		}
		...
		if (count_obj(shipping_real_data) == 0 && form_data['field']['appoint_free_shipping'] == 1) {
			layer.msg('至少要设置一个免邮地区');
			return false;
		}
		...
		var data_json = {
			...
			'shipping_json': JSON.stringify(shipping_real_data),
			'template_id': template_id,
			'surplus_area_ids': surplus_area_ids,
			'shipping_surplus_area_ids': shipping_surplus_area_ids,
			'appoint_free_shipping' : form_data['field']['appoint_free_shipping']
		};
	...
	function initshippingSubmitData(){
		var data = {};
		var num = 1;
		$('#shippingArea .area_ids').each(function (i, e) {
			var other = {};
			other['snum'] = $(e).attr('data-snum');
			other['sprice'] = $(e).attr('data-sprice');
			data[num] = {};
	 ...
	 * 开始添加免邮操作
	 */
	$("#shippingArea").on("click", ".js-add-record", function () {
		...
		layer_index = layer.open({
			...
			type: 1,
			area: ['700px', '542px'], //宽高
			content: get_shipping_modal_html(),
		});
		shipping_opt_type = 1;
		// 重置临时数据对象
		shipping_temp_area = new Object();
	 ...
	 * 获取弹框html
	 */
	function get_shipping_modal_html() {
		var modal_html = '';
		modal_html += '<div class="area-modal shipping-area-modal">' +
			'<div class="shipping-area-list area-list all-area">' +
			'<div class="title ">可选省、市、区</div>' +
			'<div class="box"></div>' +
			'</div>' +
			'<button class="add">添加</button>' +
			'<div class="shipping-area-list area-list selected-area">' +
			'<div class="title ">已选省、市、区</div>' +
			'<div class="box"></div>' +
			'</div>' +
			'</div>' +
			'<div class="modal-operation">' +
			'<button class="layui-btn save-btn shipping-save-btn">确定</button>' +
			'<button class="layui-btn layui-btn-primary" onclick="back()">返回</button>' +
			'</div>';
		return modal_html;
	}
	 ...
	 * 渲染列表
	 */
	function shipping_list(area_list, level, pid, html) {
		if (!area_list[level] || !area_list[level][pid]) return html;
		html += '<ul>';
		for (var id in area_list[level][pid]['child_list']) {
			var item = area_list[level][pid]['child_list'][id];
			...
			shipping_area_target[level] = item.id;
			shipping_area_target = shipping_area_target.slice(0, level + 1);
			html += '<li class="' + selected_html + '" data-level="' + item.level + '" data-pid="' + item.pid + '" data-id="' + item.id + '" data-extended="' + extended + '" data-selected="' + selected + '" data-target="' + shipping_area_target.toString() + '">';
			html += '<div class="title-div" >';
			...
			html += '<div class="area-delete">×</div>';
			html += '</div>';
			if (extended == 1 && has_num > 0) html += shipping_list(area_list, level + 1, id, '');
			html += '</li>';
		}
		html += '</ul>';
		return html;
	...
	function shipping_data() {
		var surplus_html = shipping_list(shipping_surplus_area, 1, 0, '');
		$(".shipping-area-modal .all-area .box").html(surplus_html);
		var temp_html = shipping_list(shipping_temp_area, 1, 0, '');
		$(".shipping-area-modal .selected-area .box").html(temp_html);
	}

	/**
	 * 展开与收起 思路为先改变数据 再重新渲染
	 */
	$("body").on('click', '.shipping-area-list .area-btn', function () {
		var li = $(this).parent().parent();
		var extended = li.attr('data-extended');
		var level = li.attr('data-level');
		var pid = li.attr('data-pid');
		...
		var alter_res = extended == 1 ? 0 : 1;
		if (shipping_surplus_area[level] && shipping_surplus_area[level][pid] && shipping_surplus_area[level][pid]['child_list'][id]) shipping_surplus_area[level][pid]['child_list'][id]['extended'] = alter_res;
		if (shipping_temp_area[level] && shipping_temp_area[level][pid] && shipping_temp_area[level][pid]['child_list'][id]) shipping_temp_area[level][pid]['child_list'][id]['extended'] = alter_res;
		shipping_data();
	});
	 ...
	 * 选中与取消 思路同展开与收起 也是先改变数据 再重新渲染
	 */
	$("body").on('click', '.shipping-area-list.all-area ul li .title-div .name-div', function () {
		...
		shipping_surplus_area[level][pid]['child_list'][id]['selected'] = alter_res;
		shipping_surplus_area[level][pid]['child_list'][id]['choosed'] = alter_res;
		area_down_alter(shipping_surplus_area, Number(level), id, alter_res, 'selected');
		area_up_alter(shipping_surplus_area, Number(level), target, 'selected');
		shipping_data();
	});
	 ...
	 * 将选中部分添加到已选中 思路为先遍历和处理数据 再渲染两边的列表
	 */
	$("body").on("click", ".shipping-area-modal .add", function () {
		var choose = deal_data(shipping_surplus_area, 'selected');
		combine_data(choose, shipping_temp_area);

		// 转换地区id
		shipping_surplus_area_ids = JSON.stringify(get_area_ids(shipping_surplus_area));
		shipping_data();
	});
	 ...
	 * 思路：相当于是执行了两步操作 1：选中 2：反向添加
	 */
	$("body").on("click", ".shipping-area-modal .shipping-area-list.selected-area .area-delete", function () {
		...
		shipping_temp_area[level][pid]['child_list'][id]['selected'] = alter_res;
		shipping_temp_area[level][pid]['child_list'][id]['choosed'] = alter_res;
		area_down_alter(shipping_temp_area, Number(level), id, alter_res, 'selected');
		area_up_alter(shipping_temp_area, Number(level), target, 'selected');

		// 找到 删除 合并
		var choose = deal_data(shipping_temp_area, 'selected');

		combine_data(choose, shipping_surplus_area);
		shipping_data();
		//转换地区id
		shipping_surplus_area_ids = JSON.stringify(get_area_ids(shipping_surplus_area));
	});
	 ...
	 * 保存操作
	 */
	$("body").on("click", ".shipping-save-btn", function () {
		layer.close(layer_index);
		if (count_obj(shipping_temp_area) == 0) return;
		if (shipping_opt_type == 1) {
			shipping_opt_total++;
			shipping_opt_num = shipping_opt_total;
			// 选中地区数据
			shipping_selected_area[shipping_opt_num] = new Object();

			// 提交表单数据
			shipping_submit_data[shipping_opt_num] = new Object();
			shipping_submit_data[shipping_opt_num]['other'] = {
				'snum': 1.00,
				'sprice': 0.00,
			};
		}
		shipping_submit_data[shipping_opt_num]['area_ids'] = new Object();
		// 把临时数据变为正式数据
		for (var per in shipping_temp_area) {
			shipping_selected_area[shipping_opt_num][per] = shipping_temp_area[per];
		}
		shipping_submit_data[shipping_opt_num]['area_names'] = alter_text(shipping_temp_area, 1, 0, '')['html'];

		// 编译数据表格
		shipping_compile_table();

		// 获取剩余地区和选中地区的ID数据
		shipping_surplus_area_ids = JSON.stringify(get_area_ids(shipping_surplus_area));
		shipping_submit_data[shipping_opt_num]['area_ids'] = JSON.stringify(get_area_ids(shipping_temp_area));
	});

	/**
	 * 取消操作
	 */
	$("body").on("click", ".shipping-area-modal .cancel-btn", function () {
		layer.close(layer_index);
	});
	 ...
	 * 删除操作
	 */
	$("#shippingArea").on("click", ".opt-delete", function () {
		// 确定正在删除的数据序列是哪一个
		shipping_opt_num = parseInt($(this).attr('data-selected'));
		//询问框
		layer_index = layer.confirm('确定要删除该记录吗', {
			btn: ['确定', '取消'] //按钮
		}, function () {
			alter_data_attr(shipping_selected_area[shipping_opt_num], 'choosed', 0);
			// 归还数据到总数据
			combine_data(shipping_selected_area[shipping_opt_num], shipping_surplus_area);
			//转换地区id
			shipping_surplus_area_ids = JSON.stringify(get_area_ids(shipping_surplus_area));
			// 销毁当前数据
			delete shipping_selected_area[shipping_opt_num];

			// 销毁提交数据
			delete shipping_submit_data[shipping_opt_num];

			// 删除当前行
			$("#shippingArea tr[data-selected='" + shipping_opt_num + "']").remove();
			layer.close(layer_index);
		}, function () {
	 ...
	 * 2：在刚进入页面的时候初始化数据
	 */
	function shipping_compile_table() {
		var html = '';
		for (var index in shipping_submit_data) {
			if (count_obj(shipping_submit_data[index]) > 0 && shipping_submit_data[index]['area_names']) {
				html += '<tr data-selected="' + index + '">';
				html += '<td>';
				html += '<p class="area-show">' + shipping_submit_data[index]['area_names'];
				html += '<span class="right-opt"><span class="opt-update shipping-opt-update text-color" data-selected="' + index + '">修改</span>&nbsp;<span class="opt-delete text-color" data-selected="' + index + '">删除</span></span>';
				html += '</p>';
				html += '</td>';
				for (var per in shipping_submit_data[index]['other']) {
					html += '<td>';
					html += '<div class="layui-input-inline">';
					html += '<input type="text" name="' + per + '" data-selected="' + index + '" class="layui-input" value="' + shipping_submit_data[index]['other'][per] + '">';
					html += '</div>';
					html += '</td>';
				}
				html += '</td>';
		...
		html += '<tr>';
		html += '<td colspan="5" class="text-color add-distribution js-add-record">指定包邮区域</td>';
		html += '</tr>';
		$("#shippingArea tbody").html(html);
	}

	/**
	 * 开始修改操作
	 */
	$("#shippingArea").on("click", ".shipping-opt-update", function () {
		// 操作类型为添加
		shipping_opt_type = 2;
		// 确定正在操作的数据序列
		shipping_opt_num = parseInt($(this).attr('data-selected'));
		// 将临时数据对象清零并重新赋值
		shipping_temp_area = new Object();
		shipping_temp_area = copy_obj(shipping_selected_area[shipping_opt_num]);
		//打开弹框
		layer_index = layer.open({
			title: '编辑包邮区域',
			type: 1,
	 ...
	 * 注：中间任何一步出错都初始化数据
	 */
	$("#shippingArea").on("change", "input", function () {
		var value = $(this).val();
		var name = $(this).attr('name');
		var selected_num = $(this).attr('data-selected');
		...
		if (value < 0) value *= -1;
		shipping_submit_data[selected_num]['other'][name] = value;
		$(this).val(value);
